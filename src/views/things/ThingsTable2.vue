...
// FIXME: this needs a consumed Thing as the TD doesn't hold values
const getThingName = (td:ThingTD): string => {
  let tdProp = ThingTD.GetThingProperty(td, PropNameName)
  if (!tdProp) {
    return td.title
  }
  // return tdProp.value ? tdProp.value : td.title
  return td.title
}
...
const columns: Array<ISimpleTableColumn> = [
  ...
  {title: "Name", field:"title" , align:"left", width:"200px",
